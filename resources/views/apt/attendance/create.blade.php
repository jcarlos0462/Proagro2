...
<head>
    ...
    <title>Nuevo Control de Asistencias - GLS-AP-FO-005</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .btn-small-green { background: var(--emerald-600); color: #fff; }
        .btn-small-green:hover { background: var(--emerald-700); }
            ...
            <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 20px;">
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <a href="{{ route('apt.attendance.index') }}" class="btn-nav">
                        <i class="fa-solid fa-arrow-left"></i> Volver a Prestadores de Servicios
                    </a>
                </div>
                <div>
                    <span style="display: inline-block; background: var(--emerald-50); color: var(--emerald-700); border: 1px solid var(--emerald-200); padding: 6px 14px; border-radius: 999px; font-size: 12px; font-weight: 900; text-transform: uppercase;">
                        Formato Oficial GLS-AP-FO-005
                    </span>
                </div>
            </div>

            <form method="POST" action="{{ route('apt.attendance.store') }}" class="form-card" id="attendanceForm">
                @csrf
                <input type="hidden" name="format_code" value="GLS-AP-FO-005">
                <input type="hidden" name="supervision_name" value="PRO-AGROINDUSTRIA, S.A. DE C.V.">
                    ...
                    <div style="display: flex; align-items: center; gap: 16px;">
                        ...
                        <div>
                            <span style="font-size: 11px; font-weight: 900; letter-spacing: 0.1em; text-transform: uppercase; background: rgba(255,255,255,0.2); padding: 3px 8px; border-radius: 999px;">
                                APT · Formato GLS-AP-FO-005
                            </span>
                            <h2>Control de Asistencias de Personal</h2>
                            <p>Registro operativo de turno y prestadores de servicio</p>
                        </div>
                    </div>
                    <div>
                        <button type="submit" class="btn-primary">
                            <i class="fa-solid fa-floppy-disk"></i> Guardar e Imprimir Formato
                        </button>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid-4">
                        <div style="grid-column: span 2;">
                            ...
                            <select name="service_provider" required class="form-control">
                                <option value="">Seleccione una empresa...</option>
                                @foreach(['OBRAS Y SERVICIOS INDUSTRIALES SAN MARTIN, SA DE CV', 'SERVICIOS LOGISTICOS Y PORTUARIOS DEL GOLFO SA DE CV', 'OPERADORA PORTUARIA INTEGRAL', 'PERSONAL PROPIO PROAGROINDUSTRIA'] as $provider)
                                    <option value="{{ $provider }}" @selected(old('service_provider') === $provider)>{{ $provider }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="work_area" required class="form-control">
                                ...
                                @foreach(['APT 2', 'APT 1', 'APT 3', 'APT 4', 'APT 5', 'Muelle APT', 'Área de Envasado'] as $area)
                                    <option value="{{ $area }}" @selected(old('work_area') === $area)>{{ $area }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div style="grid-column: span 2;">
                            ...
                            <select name="activity" required class="form-control">
                                ...
                                @foreach(['ENVASADO DE UREA EN SACO DE 25 Y ESTIBADO CAMION', 'ENVASADO DE UREA EN SACO DE 50 Y ESTIBADO CAMION', 'CARGA DE UREA A GRANEL EN CAMION', 'DESCARGA Y ESTIBADO DE PRODUCTO TERMINADO', 'RECEPCION Y ACOMODO DE SACOS EN ALMACEN', 'LIMPIEZA, ORDEN Y MANTENIMIENTO EN AREA APT'] as $activity)
                                    <option value="{{ $activity }}" @selected(old('activity') === $activity)>{{ $activity }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="loading_line" value="{{ old('loading_line', 'GLS-APT-ENV (   )') }}" class="form-control" placeholder="ej. GLS-APT-ENV (   )">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 1rem;">
                        <h3 class="section-title" style="margin: 0;">
                            <i class="fa-solid fa-users"></i> 2. Personal que Realiza la Actividad
                        </h3>
                                ...
                                @for($i = 1; $i <= 12; $i++)
                                    ...
                                    <tr id="row-{{ $i }}">
                                        <td style="text-align: center;">
                                            <input type="text" name="personnel[{{ $i - 1 }}][num]" value="{{ $num }}" class="table-input table-input-center" style="font-weight: 800; color: var(--slate-600);">
                                        </td>
                                        <td>
                                            <input type="text" name="personnel[{{ $i - 1 }}][name]" value="" placeholder="Nombre trabajador #{{ $num }}" class="table-input" style="text-transform: uppercase;">
                                        </td>
                                        <td>
                                            <input type="text" name="personnel[{{ $i - 1 }}][signature]" value="" placeholder="Firma física" class="table-input table-input-center">
                                        </td>
                                        <td>
                                            <input type="text" name="personnel[{{ $i - 1 }}][entry_time]" value="07:00" class="table-input table-input-center entry-time-input" style="font-weight: 700;">
                                        </td>
                                        <td>
                                            <input type="text" name="personnel[{{ $i - 1 }}][exit_time]" value="19:00" class="table-input table-input-center exit-time-input" style="font-weight: 700;">
                                        </td>
                                        <td style="text-align: center;">
                    ...
                    <div class="form-grid-2">
                        ...
                        <div class="leader-card">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                ...
                                <button type="button" class="btn-small btn-small-blue" onclick="syncSquadLeader()">
                                    Copiar trabajador #1
                                </button>
                            </div>
                            ...
                            <div style="margin-bottom: 10px;">
                                ...
                                <input type="text" name="squad_leader[0][name]" id="leaderName" value="" placeholder="ej. NAHUM ANTONIO MORALES MONTEJO" class="form-control" style="text-transform: uppercase;">
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <div>
                                    <label class="form-label">H.E (Entrada)</label>
                                    <input type="text" name="squad_leader[0][entry_time]" id="leaderEntry" value="07:00" class="form-control" style="text-align: center; font-weight: 700;">
                                </div>
                                <div>
                                    <label class="form-label">H.S (Salida)</label>
                                    <input type="text" name="squad_leader[0][exit_time]" id="leaderExit" value="19:00" class="form-control" style="text-align: center; font-weight: 700;">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="leader-card">
                            ...
                            <div style="margin-bottom: 10px;">
                                ...
                                <input type="text" name="safety_supervisor[0][name]" value="" placeholder="ej. ING. MARIA JOSE MARTINEZ SALINAS" class="form-control" style="text-transform: uppercase;">
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                <div>
                                    <label class="form-label">H.E (Entrada)</label>
                                    <input type="text" name="safety_supervisor[0][entry_time]" value="07:00" class="form-control" style="text-align: center; font-weight: 700;">
                                </div>
                                <div>
                                    <label class="form-label">H.S (Salida)</label>
                                    <input type="text" name="safety_supervisor[0][exit_time]" value="19:00" class="form-control" style="text-align: center; font-weight: 700;">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <textarea name="observations" rows="3" class="form-control" placeholder="Escribe aquí cualquier incidencia o nota operativa..."></textarea>
                    ...
                    <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--slate-200);">
                        <div style="font-size: 13px; color: var(--slate-500); font-weight: 600;">
                            Supervisión: <strong style="color: var(--slate-800);">PRO-AGROINDUSTRIA, S.A. DE C.V.</strong>
                        </div>
                        <div style="display: flex; gap: 10px;">
                            ...
                            <button type="submit" class="btn-primary">
                                <i class="fa-solid fa-floppy-disk"></i> Guardar y Generar Documento Imprimible
                            </button>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function syncSquadLeader() {
        const firstWorkerName = document.querySelector('input[name="personnel[0][name]"]')?.value;
        const firstWorkerEntry = document.querySelector('input[name="personnel[0][entry_time]"]')?.value;
        const firstWorkerExit = document.querySelector('input[name="personnel[0][exit_time]"]')?.value;

        if (firstWorkerName) {
            document.getElementById('leaderName').value = firstWorkerName;
        }
        if (firstWorkerEntry) {
            document.getElementById('leaderEntry').value = firstWorkerEntry;
        }
        if (firstWorkerExit) {
            document.getElementById('leaderExit').value = firstWorkerExit;
        }
    }
</script>
</body>
