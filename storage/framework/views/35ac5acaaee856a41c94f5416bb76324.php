...
<body>
    <main class="page">
        <a class="back" href="<?php echo e(route('apt.production.hub')); ?>"><i class="fa-solid fa-arrow-left"></i> Volver a Gestión de la producción</a>
        ...
        <form class="filters" method="GET" action="<?php echo e(route('apt.management.assignments.report')); ?>">
            <label class="field"><span>Buscar usuario, lote o turno</span><input type="search" name="search" value="<?php echo e($search); ?>" placeholder="Ej. nombre o folio"></label>
            <label class="field"><span>Estado</span><select name="status"><option value="all" <?php if($status === 'all'): echo 'selected'; endif; ?>>Todos</option><option value="open" <?php if($status === 'open'): echo 'selected'; endif; ?>>Abiertos</option><option value="closed" <?php if($status === 'closed'): echo 'selected'; endif; ?>>Cerrados</option></select></label>
            <button class="button" type="submit"><i class="fa-solid fa-filter"></i> Consultar</button>
            <a class="clear" href="<?php echo e(route('apt.management.assignments.report')); ?>">Limpiar</a>
        </form>

        <section class="summary" aria-label="Resumen de asignaciones">
            <div class="summary-card"><div class="summary-icon"><i class="fa-solid fa-users"></i></div><div><span class="summary-value"><?php echo e($summary['users']); ?></span><span class="summary-label">Usuarios</span></div></div>
            <div class="summary-card"><div class="summary-icon"><i class="fa-solid fa-boxes-stacked"></i></div><div><span class="summary-value"><?php echo e($summary['total']); ?></span><span class="summary-label">Asignaciones</span></div></div>
            <div class="summary-card"><div class="summary-icon"><i class="fa-solid fa-lock-open"></i></div><div><span class="summary-value"><?php echo e($summary['open']); ?></span><span class="summary-label">Lotes abiertos</span></div></div>
            <div class="summary-card"><div class="summary-icon"><i class="fa-solid fa-circle-check"></i></div><div><span class="summary-value"><?php echo e($summary['closed']); ?></span><span class="summary-label">Lotes cerrados</span></div></div>
        </section>

        <section class="table-wrap">
            <div class="table-head"><h2>Detalle por usuario</h2><span><?php echo e($summary['total']); ?> asignaciones encontradas</span></div>
            <div class="scroll">
                <table>
                    ...
                    <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $userAssignments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $userAssignment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php $firstAssignment = $userAssignment['lots']->first(); ?>
                        <tr data-user-row>
                            <td class="user"><?php echo e($userAssignment['user_name']); ?></td>
                            <td>
                                <select class="lot-select" data-assignment-select aria-label="Lotes asignados a <?php echo e($userAssignment['user_name']); ?>">
                                    <?php $__currentLoopData = $userAssignment['lots']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $assignment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($loop->index); ?>"
                                            data-folio="<?php echo e($assignment->folio); ?>"
                                            data-warehouse="<?php echo e($assignment->warehouse ?: 'Sin disposición'); ?>"
                                            data-shift="<?php echo e($assignment->shift); ?>"
                                            data-started-date="<?php echo e($assignment->started_at ? \Carbon\Carbon::parse($assignment->started_at)->format('d/m/Y') : '—'); ?>"
                                            data-started-time="<?php echo e($assignment->started_at ? \Carbon\Carbon::parse($assignment->started_at)->format('H:i:s') : '—'); ?>"
                                            data-status="<?php echo e($assignment->status); ?>"
                                            data-closed-date="<?php echo e($assignment->closed_at ? \Carbon\Carbon::parse($assignment->closed_at)->format('d/m/Y') : '—'); ?>"
                                            data-closed-time="<?php echo e($assignment->closed_at ? \Carbon\Carbon::parse($assignment->closed_at)->format('H:i:s') : 'Pendiente'); ?>"
                                            data-closed-by="<?php echo e($assignment->closed_by_name ?: '—'); ?>">
                                            <?php echo e($assignment->folio); ?><?php echo e($assignment->status === 'closed' ? ' · Cerrado' : ' · Abierto'); ?>

                                        </option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                                <span class="sub" data-assignment-warehouse><?php echo e($firstAssignment->warehouse ?: 'Sin disposición'); ?></span>
                            </td>
                            <td data-assignment-shift><?php echo e($firstAssignment->shift); ?></td>
                            <td><span data-assignment-started-date><?php echo e($firstAssignment->started_at ? \Carbon\Carbon::parse($firstAssignment->started_at)->format('d/m/Y') : '—'); ?></span><span class="sub" data-assignment-started-time><?php echo e($firstAssignment->started_at ? \Carbon\Carbon::parse($firstAssignment->started_at)->format('H:i:s') : '—'); ?></span></td>
                            <td><span class="badge <?php echo e($firstAssignment->status === 'closed' ? 'closed' : 'open'); ?>" data-assignment-status><i class="fa-solid <?php echo e($firstAssignment->status === 'closed' ? 'fa-check' : 'fa-clock'); ?>"></i><?php echo e($firstAssignment->status === 'closed' ? 'Cerrado' : 'Abierto'); ?></span></td>
                            <td><span data-assignment-closed-date><?php echo e($firstAssignment->closed_at ? \Carbon\Carbon::parse($firstAssignment->closed_at)->format('d/m/Y') : '—'); ?></span><span class="sub" data-assignment-closed-time><?php echo e($firstAssignment->closed_at ? \Carbon\Carbon::parse($firstAssignment->closed_at)->format('H:i:s') : 'Pendiente'); ?></span></td>
                            <td data-assignment-closed-by><?php echo e($firstAssignment->closed_by_name ?: '—'); ?></td>
                        </tr>
...
<?php /**PATH C:\xampp\htdocs\Proagroindustria2\resources\views/production/assignments-report.blade.php ENDPATH**/ ?>