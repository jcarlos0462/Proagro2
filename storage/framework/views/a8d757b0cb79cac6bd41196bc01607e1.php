...
<div class="page">
    <div class="nav-row">
        <a class="back-btn" href="<?php echo e($isAlmacenista ? route('apt.production') : route('apt.management')); ?>">
    ...
    <main class="panel" data-shift-id="<?php echo e($registration->id ?? ''); ?>">
        <section class="card">
            ...
            <label class="field">Usuario asignado<input readonly value="<?php echo e(optional(optional($registration)->user)->name ?: auth()->user()->name); ?>"></label>
            <label class="field">Puesto<input readonly value="<?php echo e(optional($registration)->position ?: (auth()->user()->position ?: 'Almacén')); ?>"></label>
            <label class="field">Turno<input readonly value="<?php echo e(optional($registration)->shift ?: '—'); ?>"></label>
            <label class="field">Fecha<input readonly value="<?php echo e(optional(optional($registration)->started_at)->format('d/m/Y') ?: date('d/m/Y')); ?>"></label>
        </section>
        <section class="card">
            <div class="card-head">
            ...
            <label class="field wide">No. de lote
                <select data-activity-lot <?php echo e($selectedLots->isEmpty() ? 'disabled' : ''); ?>>
                    <?php if($selectedLots->count() > 1): ?>
                        <option value="" <?php echo e($requestedLotId === '' ? 'selected' : ''); ?>>Seleccionar lote...</option>
                    <?php endif; ?>
                    <?php $__empty_1 = true; $__currentLoopData = $selectedLots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <option value="<?php echo e($lot->id); ?>"
                            data-product="<?php echo e(preg_match('/-UI(?:[-(]|$)/i', (string) $lot->folio) ? 'UREA INDUSTRIAL' : (preg_match('/-UA(?:[-(]|$)/i', (string) $lot->folio) ? 'UREA AGRICOLA' : '—')); ?>"
                            data-origin="<?php echo e($lot->plant_origin ?: '—'); ?>"
                            data-warehouse="<?php echo e($lot->warehouse ?: '—'); ?>"
                            data-status="<?php echo e($lot->pivot?->status ?: 'open'); ?>"
                            <?php echo e(($activeLot?->id ?? null) == $lot->id ? 'selected' : ''); ?>><?php echo e($lot->folio); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <option value="">—</option>
                    <?php endif; ?>
                </select>
            </label>
            <label class="field">Producto<input data-activity-product readonly value="<?php echo e(preg_match('/-UI(?:[-(]|$)/i', (string) $activeLot?->folio) ? 'UREA INDUSTRIAL' : (preg_match('/-UA(?:[-(]|$)/i', (string) $activeLot?->folio) ? 'UREA AGRICOLA' : '—')); ?>"></label>
            <label class="field">Origen<input data-activity-origin readonly value="<?php echo e($activeLot?->plant_origin ?: '—'); ?>"></label>
            <label class="field">Disposición<input data-activity-warehouse readonly value="<?php echo e($activeLot?->warehouse ?: '—'); ?>"></label>
            ...
            <label class="field">Status de Producción
                <input data-production-close-status
                    class="<?php echo e(($activeLot?->pivot?->status ?: 'open') === 'closed' ? 'status-closed' : 'status-open'); ?>"
                    readonly
                    value="<?php echo e(($activeLot?->pivot?->status ?: 'open') === 'closed' ? 'Cerrado' : 'Abierto'); ?>">
            </label>
            <?php if($registration): ?>
                <?php if($canCloseLot ?? false): ?>
        ...
        <section class="card history">
            ...
            <div class="history-filters">
                ...
                <label class="filter-date-label">Fecha
                    <input type="date" data-history-date value="<?php echo e($activityDate ?? ''); ?>" onclick="try{this.showPicker()}catch(e){}">
                </label>
            ...
            <table>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $activities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $activity): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        ?>
                        <tr>
                            <td><?php echo e($activity->occurred_at ? $activity->occurred_at->format('d/m/Y') : '—'); ?></td>
                            <td><?php echo e($activity->occurred_at ? $activity->occurred_at->format('H:i:s') : '—'); ?></td>
                            <td><?php echo e(optional($displayLot)->folio ?: '—'); ?></td>
                            <td><?php echo e($displayWarehouse); ?></td>
                            <td><?php echo e($activity->location ?: 'Automático'); ?></td>
                            <td><?php echo e($displayProduct); ?></td>
    ...
    <main class="panel-waiting">
        <div class="waiting-card">
            ...
            <div class="waiting-user-pill">
                <div class="user-avatar-mini"><?php echo e(mb_substr(auth()->user()->name, 0, 1)); ?></div>
                <div class="user-meta-mini">
                    <strong><?php echo e(auth()->user()->name); ?></strong>
                    <span><?php echo e(auth()->user()->position ?: 'Operador de Almacén'); ?></span>
                </div>
            </div>
        </div>
    </main>
...
<script>
    (() => {
        ...
        printBtn?.addEventListener('click', () => {
            const printUrl = new URL('<?php echo e(route('apt.management.activity.print')); ?>', window.location.origin);
            if (shiftId) {
        ...
        if (waitingPanel) {
            ...
            const checkAssignment = async () => {
                ...
                isChecking = true;
                try {
                    const checkUrl = "<?php echo e(route('apt.management.check.assigned.lots')); ?>";
...
<?php /**PATH C:\xampp\htdocs\Proagroindustria2\resources\views/production/activity.blade.php ENDPATH**/ ?>